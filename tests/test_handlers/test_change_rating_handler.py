...
from tests.conftest import CHANGE_RATING_URL
from utils.roles import PortalRole


async def test_change_user_rating(client, create_user_in_database, get_user_from_database):
    user_data = {
        "user_id": uuid4(),
        "name": "Nikolai",
        "surname": "Sviridov",
    ...
    resp = client.post(
        f"{USER_URL}{CHANGE_RATING_URL}?user_id={user_data['user_id']}",
        headers=await create_test_auth_headers_for_user(admin_data["email"]),
        json=user_rating_updated,
    )
    ...
    assert user_from_db["rating"] == user_rating_updated["rating"]


@pytest.mark.parametrize(
    "user_id, user_changed_rating, expected_status_code, expected_detail",
    [
        (
            "",
            {},
        ...
        (
            ...
            400,
            {
                "detail": "Rating can be from 0 to 80."
            },
        ),
        (
            "06815f2f-0944-78b6-8000-8cb811cc4533",
...
async def test_change_user_rating_validation_error(
    client,
    ...
    resp = client.post(
        f"{USER_URL}{CHANGE_RATING_URL}?user_id={user_id}",
        headers=await create_test_auth_headers_for_user(admin_data["email"]),
        json=user_changed_rating,
    )
...
async def test_change_user_rating_bad_cred(client, create_user_in_database):
    user_data = {
        "user_id": uuid4(),
        "name": "Nikolai",
        "surname": "Sviridov",
...
async def test_change_user_rating_unauth(client, create_user_in_database):
    user_data = {
        "user_id": uuid4(),
        "name": "Nikolai",
        "surname": "Sviridov",
...
async def test_change_user_rating_no_jwt(client, create_user_in_database):
    user_data = {
        "user_id": uuid4(),
        "name": "Nikolai",
        "surname": "Sviridov",
...
async def test_change_user_rating_by_privilage_roles(
...
async def test_change_user_rating_privilage_error(
    client,
    create_user_in_database,
    ...
    reps = client.post(
        f"{USER_URL}{CHANGE_RATING_URL}?user_id={user_data_for_changing['user_id']}",
        json=updated_rating,
